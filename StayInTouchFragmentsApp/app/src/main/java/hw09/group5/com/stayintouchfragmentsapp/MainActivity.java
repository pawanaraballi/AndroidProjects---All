...
ArchiveFragment.OnFragmentInteractionListener{
    ...
    public void returnEmail(String string) {
        ...
        Firebase ssss = new Firebase("https://hw09-stayintouch-group05.firebaseio.com/");

        ssss.child("users/username").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                for (DataSnapshot postSnapshot : dataSnapshot.getChildren()) {
                    User iteratoruser = postSnapshot.getValue(User.class);
                    if (iteratoruser.getUserEmail().equalsIgnoreCase(returnedEmail)) {
                        currUser = iteratoruser;
                    }
                }
                /*ImageView imageView = (ImageView) findViewById(R.id.imageView_ViewContactdp_menu);

                byte[] decodedString = Base64.decode(currUser.getProfilepic(), Base64.DEFAULT);
    ...
    public void selectDrawerItem(MenuItem menuItem) {
        ...
        switch(menuItem.getItemId()) {
            ...
            case R.id.nav_archive_fragment:
                ...
                String emailArchive = (String) ref.getAuth().getProviderData().get("email");
                Bundle bundleArchive = new Bundle();
                bundleArchive.putString("email", emailArchive);
                ConversationFragment fragmentArc = new ConversationFragment();
                fragmentArc.setArguments(bundleArchive);
                getSupportFragmentManager().beginTransaction().replace(R.id.container, fragmentArc , "ArchiveList")
                        .addToBackStack(null)
                        .commit();
                break;
